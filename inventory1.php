                            ...
                            <div class="sm:col-span-6 border-b border-gray-200 pb-4 mb-4">
                                ...
                                <div class="grid grid-cols-1 gap-y-4 gap-x-6 sm:grid-cols-6">
                                    <div class="sm:col-span-6">
                                        <label for="item-name" class="block text-sm font-medium text-gray-700 flex items-center">
                                    ...
                                    <div class="sm:col-span-3">
                                        <label for="item-category" class="block text-sm font-medium text-gray-700 flex items-center">
                                    ...
                                    <div class="sm:col-span-3">
                                        <label for="item-quantity" class="block text-sm font-medium text-gray-700 flex items-center">
                                    ...
                                    <div class="sm:col-span-3">
                                        <label for="item-unit" class="block text-sm font-medium text-gray-700 flex items-center">
            ...
            <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full">
                ...
                <div class="max-h-[calc(100vh-200px)] overflow-y-auto px-6 py-4">
                    ...
                    <div class="border-t border-gray-200 pt-4">
                        ...
                        <div id="item-activities" class="space-y-4">
                ...
                <!-- Fixed Footer -->
                <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse border-t border-gray-200 sticky bottom-0">
                    <button type="button" id="edit-item-from-details" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-blue-600 text-base font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 sm:ml-3 sm:w-auto sm:text-sm">
    ...
    <script>
        ...
        function setupUI() {
            ...
            document.getElementById('save-item').addEventListener('click', async () => {
                ...
                try {
                    ...
                    showErrorToast('Error saving item. Please try again.');
                }
            });
            ...
            document.getElementById('next-page').addEventListener('click', async (e) => {
                e.preventDefault();
                const totalItems = await getTotalItemsCount();
            ...
            // Edit from details
            document.getElementById('edit-item-from-details').addEventListener('click', () => {
                const id = document.getElementById('item-details-modal').getAttribute('data-item-id');
                if (id) {
        ...
        function displaySearchResults(results) {
            ...
            searchResults.innerHTML = results.slice(0, 5).map(item => `
                <a href="#" class="search-item block hover:bg-gray-50 transition-colors duration-150" data-item-id="${item.id}">
                    <div class="flex items-center p-3">
                        ...
                        <div class="ml-3 flex-1 min-w-0">
                            ...
                            <p class="text-xs text-gray-500 truncate">${item.category || 'No category'} • ${getStatusText(item.status)}</p>
                        </div>
                    </div>
                </a>
            `).join('');
            ...
            document.querySelectorAll('.search-item[data-item-id]').forEach(item => {
...
function addItemActionListeners() {
    ...
    document.querySelectorAll('.delete-item').forEach(btn => {
        ...
        function renderCardView(items = null) {
            ...
            if (!items) {
                // If no items provided, use the current paginated items
                const startIdx = (currentPage - 1) * itemsPerPage;
                items = currentItems.slice(startIdx, startIdx + itemsPerPage);
            }
            ...
            if (items.length === 0) {
                cardView.innerHTML = `
                    <div class="sm:col-span-3 text-center py-10">
                        ...
                        <button id="add-item-empty-card" class="mt-3 text-blue-600 hover:text-blue-800">Add a new item</button> to get started.
                    </div>
                `;
                
                document.getElementById('add-item-empty-card').addEventListener('click', () => {
                    document.getElementById('add-item-btn').click();
                });
                ...
                <div class="inventory-card bg-white overflow-hidden shadow rounded-lg">
                    ...
                    <div class="px-4 py-5 sm:p-6">
                        <div class="flex items-center justify-between">
                            ...
                            <span class="text-sm text-gray-500 capitalize">${item.category || 'No category'}</span>
                        </div>
                        ...
                        <div class="mt-4">
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <p class="text-sm font-medium">${item.quantity || '0'} ${item.unit || ''}</p>
                                </div>
                                <div>
                                    <p class="text-xs text-gray-500">Last Updated</p>
                                    <p class="text-sm font-medium">${formatDate(item.updated_at) || '-'}</p>
                                </div>
                            </div>
                        </div>
                        
                        <div class="mt-4 flex justify-between">
                            <button data-id="${item.id}" class="view-item inline-flex items-center px-3 py-1 border border-gray-300 shadow-sm text-sm leading-4 font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                                <i class="fas fa-eye mr-1"></i> View
                            </button>
                            <div>
                                <button data-id="${item.id}" class="edit-item inline-flex items-center px-3 py-1 border border-gray-300 shadow-sm text-sm leading-4 font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                                    <i class="fas fa-edit mr-1"></i> Edit
                                </button>
                                <button data-id="${item.id}" class="delete-item ml-2 inline-flex items-center px-3 py-1 border border-transparent text-sm leading-4 font-medium rounded-md text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
        ...
        async function openEditModal(id) {
            try {
                ...
                showErrorToast('Failed to load item for editing');
            }
        }
        ...
        async function showItemDetails(id) {
            try {
                ...
                // Set modal data
                document.getElementById('item-details-modal').setAttribute('data-item-id', item.id);
                document.getElementById('item-details-title').textContent = item.name;
                document.getElementById('item-details-subtitle').textContent = `${item.category ? item.category.charAt(0).toUpperCase() + item.category.slice(1) : ''}`;

                // Populate all the details
                document.getElementById('item-details-name').textContent = item.name;
                document.getElementById('item-details-category').textContent = item.category ? item.category.charAt(0).toUpperCase() + item.category.slice(1) : '-';
                document.getElementById('item-details-quantity').textContent = `${item.quantity || '0'} ${item.unit || ''}`;
                document.getElementById('item-details-unit').textContent = item.unit || '-';
                document.getElementById('item-details-location').textContent = item.location || '-';
                document.getElementById('item-details-notes').textContent = item.notes || 'No notes available';
                document.getElementById('item-details-added-date').textContent = formatDate(item.created_at) || '-';
                document.getElementById('item-details-updated-date').textContent = formatDate(item.updated_at) || '-';

                // Set status with colored badge
                const statusElement = document.getElementById('item-details-status');
                statusElement.textContent = getStatusText(item.status);
                statusElement.className = 'mt-1 text-sm font-medium px-2 py-1 rounded-full inline-block ' + getStatusColorClass(item.status);

                // Set image
                const imageUrl = item.image || `https://source.unsplash.com/random/300x300/?${encodeURIComponent(item.category)},farm`;
                document.getElementById('item-details-image').src = imageUrl;

                // Get and display activities
                const activities = await getItemActivities(item.id);
                const activitiesContainer = document.getElementById('item-activities');

                if (activities.length === 0) {
                    activitiesContainer.innerHTML = '<div class="text-center py-4 text-sm text-gray-500">No activities recorded for this item</div>';
                } else {
                    activitiesContainer.innerHTML = activities.map(activity => `
                        <div class="activity-item flex items-start p-3 hover:bg-gray-50 rounded-lg transition-colors">
                            ...
                            <div class="ml-3 flex-1 min-w-0">
                                <p class="text-sm font-medium text-gray-900">${activity.user_name || 'System'}</p>
